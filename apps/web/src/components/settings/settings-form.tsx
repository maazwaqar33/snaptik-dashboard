...
export function SaveButton({ saving, saved, label }: { saving: boolean; saved: boolean; label: string }) {
  return (
    <button
      type="submit"
      disabled={saving}
    ...
    <form onSubmit={handleSave}>
      ...
      <div className="rounded-xl border border-white/10 bg-[#121212] px-5">
        <Field label="App Name" hint="Displayed in emails and the mobile app">
          <TextInput value={vals.appName} onChange={(v) => set('appName', v)} />
        </Field>
        ...
        <Field label="Allowed Regions" hint="ISO country codes (comma-separated)">
          <TextInput value={vals.allowedRegions} onChange={(v) => set('allowedRegions', v)} placeholder="US,UK,CA" />
        </Field>
      </div>
      <div className="mt-4 flex justify-end">
        <SaveButton saving={saving} saved={saved} label="Save Platform Settings" />
      </div>
    </form>
  );
}

// ─── Moderation section form ──────────────────────────────────────────────────
...
export function ModerationForm() {
  ...
  return (
    <form onSubmit={handleSave}>
      ...
      <div className="mt-4 flex justify-end">
        <SaveButton saving={saving} saved={saved} label="Save Moderation Settings" />
      </div>
    </form>
  );
}
    ...
    <form onSubmit={handleSave}>
      <div className="rounded-xl border border-white/10 bg-[#121212] px-5">
        <Field label="Alert Email" hint="Receives critical platform alerts (content spikes, security events)">
          <TextInput value={vals.alertEmail} onChange={(v) => set('alertEmail', v)} type="email" />
        </Field>
        <Field label="Slack Webhook URL" hint="Leave blank to disable Slack notifications">
          <TextInput value={vals.slackWebhook} onChange={(v) => set('slackWebhook', v)} placeholder="https://hooks.slack.com/…" />
        </Field>
      ...
      <div className="mt-4 flex justify-end">
        <SaveButton saving={saving} saved={saved} label="Save Notification Settings" />
      </div>
    </form>
  );
}
